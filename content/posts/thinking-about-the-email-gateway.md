...
isCJKLanguage = true
title = "关于邮件网关的思考"
description = "关于邮件网关的思考"
keywords = ["email gateway"]
date = 2023-11-06T11:18:58+08:00
authors = ["木章永"]
tags = ["email gateway"]
categories = ["email gateway"]
aliases = [
    "/posts/thinking-about-the-email-gateway/"
]
+++
...
# 网关管理
核心的模块由`SMTP Module`，`Check Module`和`Deliver Module`组成，如果只要收发邮件的话，只需要这三个模块就够了，但是处理收发邮件外，还需要能查看收发邮件的情况，能够配置规则，能把被隔离的邮件投递出去，能监控网关的运行情况....

总之，合理核心的三个模块外，还需要有其他的模块配合，才能组成一个完整可用的网关工具。

要统计邮件的收发情况，需要能将核心的三个模块的处理信息记录到一个位置，以便查询统计。理想情况下是记录下每封邮件的处理过程，从接收邮件到检测邮件到将邮件投递出去都需要记录。

记录的这些信息其实就是邮件的处理日志，这里简称为邮件日志。 邮件日志可以由三个模块按约定的格式输出，为了避免日志对程序本身的影响，由程序将日志输出到本地文件（通常有成熟的日志库），然后通过`fluent-bit`采集日志，发送到数据擦仓库存储，以便检索统计。 

`fluent-bit`目前通常是配合`ES`，不过理论上是可以有多种选择的，只要实现的对应的插件和部署了相应的服务即可。具体如何选择主要根据使用场景来决定

方便起见，假定使用`fluent-bit`采集三个模块的日志并发送到`ES`进行存储。

记录了邮件日志后，需要能查看数据。最简单的方式是通过部署一个`kibana`来查询数据。

直接使用`kibana`的问题在于对使用者的要求较高，需要了解`ES`的查询语法，考虑到还需要提供配置规则等功能，可以实现一个`web`服务，将对数据的查询和统计需要封装为用户友好的功能，并`web`服务提供对网关的配置功能。

目前来看，在三个模块之外，还需要`fluent-bit`采集日志，`ES`存储日志，`web`服务（前后端分离时需要两个服务）进行管理。

多个服务部署考虑考虑使用`Docker`，若只需要在一台机器上部署这些服务，直接使用`Docker Compose`即可，如果需要双机或者三机，少量的机器简单起见可以使用`Docker Swarm`，如果更大规模的部署，或许使用`k8s`是个更好的选择。

三种方式在部署好底层的服务后，配置方式其实差别不大，直接使用`Docker Compose`会更加简单，并且需要多台机器时，只需要修改`yaml`和`conf`即可。